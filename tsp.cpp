...
Shop::Shop(int shopnum) {				//constructor of shop class
...
void Shop::addprods(string name, int price) {				//add products to pricelist
	pricelist.push_back(make_pair(name, price));
	prods++;

}


int Shop::getprice(string item) {				//gets price of item passed
	typedef std::vector< std::pair<string, int> > pricelisttype;

	for (pricelisttype::iterator it = pricelist.begin(); it != pricelist.end(); it++) {

		string store = it->first;

		if (store.compare(item) == 0)
			return it->second;
	}
	return -1;
}
...
void Shop::showprods() {
	...
	for (pricelisttype::iterator it = pricelist.begin(); it != pricelist.end(); it++) {
		...
		string store1 = it->first;

		std::cout << "\nprod: " << it->first << " Price: " << it->second;
...
Genetic::Genetic(Graph* graph, int size_population, int generations, int mutation_rate, vector<int>& essential_nodes, bool show_population)
{
	...
	this->essential_nodes = essential_nodes;
...
void Genetic::initialPopulation(vector<int>& essential_nodes) // generates the initial population
{
	vector<int> parent;
	...
	for (int i = 0; parent.size() < graph->V; i++)
	{
	...
	/*for (int i = 0; i < essential_nodes.size(); i++) {
	parent.push_back(essential_nodes[i]);
	}
	*/
...
void Genetic::run()
{
	initialPopulation(essential_nodes); // gets initial population
